...
// --- 1. CLIENT RELATIONSHIPS (Owner/God) ---
...
export function renderMenuSection(menuItems, sortConfig, getCategoryColor, getAllergenBadges, getSortIcon, showAllergens, settings) {
    ...
    const getThemeBasedColor = (cat) => {
         ...
         const alpha=0.08+(Math.abs(hash%20)/100);
    ...
    const rows = sortedItems.map(item => `
        <tr data-item-id="${item.id}" 
            style="background-color: ${getThemeBasedColor(item.category||'')}; border-bottom:1px solid #fff; cursor:pointer;"
...
export function renderHeaderSection(headerSettings) {
    ...
    // FIX: Define bgColor to prevent ReferenceError

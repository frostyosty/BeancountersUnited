...
const OPTION_GROUPS = {
    ...
    "Food Modifications": ["Toasted", "Fresh (Not Toasted)", "No Butter", "Extra Butter", "No Sauce", "Tomato Sauce", "BBQ Sauce", "Mayo"],
...
export function showEditItemModal(item) {
    ...
    const modalHTML = `
        <div class="modal-form-container" style="max-width: 700px;">
            <h3>${isEditing ? 'Edit Item' : 'Add New Item'}</h3>
            <form id="edit-item-form">
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div class="form-row"><label>Name</label><input type="text" name="name" value="${itemData.name}" required></div>
                    <div class="form-row"><label>Category</label><select name="category" required><option value="">Select...</option>${categories.map(cat => `<option value="${cat}" ${itemData.category === cat ? 'selected' : ''}>${cat}</option>`).join('')}</select></div>
                </div>
                <div class="form-row"><label>Price ($)</label><input type="number" name="price" value="${itemData.price}" step="0.01" required style="width: 50%;"></div>
                <div class="form-row"><label>Description</label><textarea name="description" style="height:60px;">${itemData.description || ''}</textarea></div>
                ...
                <div class="form-group" style="background:#f8f9fa; padding:15px; border-radius:6px; margin-top:15px; border:1px solid #e9ecef;">
                    ...
                    <div style="display:flex; flex-wrap:wrap; gap:20px;">${groupsHTML}</div>
                </div>
...
// --- 4. IMAGE EDITOR MODAL (WITH CROPPER.JS) ---
export async function showImageEditorModal(item) {
    ...
    const modalHTML = `
        <div class="modal-form-container" style="max-width: 600px; text-align:center;">
            ...
            <div style="margin: 20px auto; width: 300px; height: 300px; background: #333; overflow: hidden; border-radius: 8px; position: relative;">
                <!-- IMPORTANT: crossorigin added to allow canvas export -->
    ...
    document.getElementById('btn-remove-bg').onclick = async () => {
        ...
        if (src.includes('placeholder')) {
            uiUtils.showToast("Cannot process placeholder image.", "error");
            return;
        }
